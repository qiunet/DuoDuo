 ...
 * 17/8/13
 */
public class WebSocketServerDecoder extends ByteToMessageDecoder implements IMessageDecoder {
	private final Logger logger = LoggerType.DUODUO_FLASH_HANDLER.getLogger();
	private final int maxReceivedLength;
	private final boolean encryption;
	public WebSocketServerDecoder(int maxReceivedLength, boolean encryption) {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		...
		if (! in.isReadable(this.getHeaderLength(protocolHeader, connectReq))) {
			// 有的探测包没有这么长. 导致报错.
			return;
		}
