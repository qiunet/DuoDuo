...
/**
 * A {@link DThreadPoolExecutor } that can additionally schedule
 * commands to run after a given delay, or to execute
 * periodically. This class is preferable to {@link Timer}
 * when multiple worker threads are needed, or when the additional
 * flexibility or capabilities of {@link DThreadPoolExecutor } (which
 ...
 *
 * <p>While this class inherits from {@link DThreadPoolExecutor}, a fewD
 ...
 * {@link DThreadPoolExecutor#execute(Runnable) execute} and
...
public class DScheduledThreadPoolExecutor
    ...
    /*
     * This class specializes DThreadPoolExecutor implementation by
     *
     ...
     *    (see delayedExecute) compared to DThreadPoolExecutor .
     *
	 ...
	 * given delay check time.
	 *
	 * @param delay the number of delay for Queue check
	 ...
	 * given delay check millis.
	 *
	 * @param delayCheckMillis the number of delay for Queue check
	 * @throws IllegalArgumentException if {@code corePoolSize < 0}
	 */
    ...
    /**
     * Creates a new {@code DScheduledThreadPoolExecutor} with the
    ...
    /**
     * Creates a new DScheduledThreadPoolExecutor with the given
     * initial parameters.
     *
     * @param corePoolSize the number of threads to keep in the pool, even
     *        if they are idle, unless {@code allowCoreThreadTimeOut} is set
    ...
    private long overflowFree(long delay) {
        ...
        if (head != null) {
            long headDelay = head.getDelay(MILLISECONDS);
     ...
     * that {@link DThreadPoolExecutor#afterExecute afterExecute} is always
    ...
    public <T> Future<T> submit(Runnable task, T result) {
        return schedule(Executors.callable(task, result), 0, MILLISECONDS);
    }
    ...
    static class DelayedWorkQueue extends AbstractQueue<Runnable>
		...
		public DelayedWorkQueue(long delayCheckMillis) {
			// set large than or equals 1 seconds
			Preconditions.checkState(delayCheckMillis >= 1000);
        ...
        public RunnableScheduledFuture<?> poll() {
            ...
            try {
                ...
                if (first == null || first.getDelay(MILLISECONDS) > 0)
                    return null;
                else
                ...
                for (;;) {
                    ...
                    if (first == null)
                        ...
                        if (leader != null)
                            ...
                            leader = thisThread;
                            try {
                                available.await(Math.min(delayCheckMillis, delay), MILLISECONDS);
                            } finally {
        ...
        private RunnableScheduledFuture<?> peekExpired() {
            ...
            RunnableScheduledFuture<?> first = queue[0];
            return (first == null || first.getDelay(MILLISECONDS) > 0) ?
                null : first;
        }
